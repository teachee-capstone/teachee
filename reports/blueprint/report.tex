...
Currently the app has three main components: \emph{USB Manager}; \emph{Storage};
and \emph{Frontend}.  The \emph{USB Manager} and \emph{Storage} components are
written in Rust and operate as separate threads in the same process; the
\emph{USB Manger} reads and writes from data in \emph{Storage} through
thread-safe getter/setter methods. The \emph{Frontend} component is written in
Javascript and operates in it's own process; it communicates with the Storage
component through message passing. JavaScript and Rust interoperability is
enabled by serializing all messages into \emph{JavaScript Object Notation}
...
\subsection{Health and safety, and legal ramifications}

As with any device, there is a risk of spontaneous catastrophic failure. For
TeachEE, there is the possibility that the device will short circuit, and cause
damage to a user's computer, or even worse, could cause a fire. In a worst case
scenario, lawsuits for wrongful death can occur.

The way to prevent these situations is to develop the device with proof of due
diligence. A rigorous test log can be produced in the event of an accusation of
negligence. As additional protection, the device can be signed off by a
