...
\subsubsection{Interface Instantiations}
...
often being manipulated during data transmission \cite{axis_spec}. Figure
\ref{fig:axis-transaction} shows the AXIS read and write timing diagrams.

\begin{figure}[h]
  \centering
  % TODO: Make the timing diagram
  % \includegraphics[width=\textwidth]{../../misc/TeachEE-System-Diagram-Hardware.png}
  \caption{AXIS Read/Write Transaction Timing Diagram}
  \label{fig:axis-transaction}
\end{figure}

In addition to the simple protocol, there is significant library support for AXI
streams. In the case of TeachEE, the \code{verilog-axis} library is used
\cite{verilog_axis_lib}.

% Also cite the GitHub page of the axis tool library.
% talk about why AXIS is great and the extensibility of the codebase as a
% a result
% Perhaps also add some timing diagram figures
% Explain the use of the library and how we write custom wrappers to bundle the
% signals together under the custom axis_interface definition
