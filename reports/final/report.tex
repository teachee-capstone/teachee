...
rising edges get triggered on \cite{hysteresis}. Triggering is done on both
...
provided function takes in the sample data, sample rate, desired frequency range,
...
\begin{enumerate}
  ...
  \item Signal plotting with cursor and various pan and zoom options
...
The testing and validation of the TeachEE consists of testing the hardware/PCB,
...
subsystems: a design component was tested in isolation and integration after being
...
two tools. The first is a mock bytestream generator that populates the channel
buffers with sawtooth waves. The generator simulates data arriving from the hardware
and allows developers to test Controller and App functionality without TeachEE
hardware. This greatly increases development flexibility as the developers could
write code and immediately test it. For example, the trigger functionality
correctness was veried on these mock bytestreams rather than the hardware. The
second tool is an automated GitHub Action script run on every branch that builds the
software, checks code formatting, and runs a linter to catch common mistakes. This
tool ensures that code merged to the main branch is written with good style and does
...
mutex, thereby starving the App thread. It also did not help that the App thread
...
prevent redundant read/writes and limit contention, and runs at sixty frames per
second on average. These frame rates were measured using instrumentation code run
...
The triggering functionality also underwent design iteration. The original
triggering algorithm uses a moving average sliding window as the noise
mitigation strategy. It requires that the average of the last \code{WINDOW_SIZE}
samples be lower than the current sample in addition to requiring that the
current sample be greater than the trigger threshold. The sliding window
...
waveforms. It did not consistently trigger at the exact same position on every
...
prompted additional decoder error handling as the bytestream channel was not
completely reliable.
...
\pagestyle{empty}
    ...
    \subsubsection{xadc\_bfm.sv} \label{xadc-bfm}
