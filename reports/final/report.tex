...
Section \ref{sec:soft-testing}. The USB manager and Controller share one set of
...
\begin{figure}[H]
  ...
  \caption{Shared Data Configuration}
  \label{fig:soft-buffer-config}
\end{figure}

In Rust, monitors are implemented using condition variables and mutexes. The condition
...
\code{Full}, it unlocks the mutex and is put to sleep until it receives a notification
...
to the appropriate type and notifies a thread waiting on the condition variable. An
example of this procedure (run by the App thread) is given in the code block below.
...
reads 60 000 bytes at a time so that the App displays approximately 10 000
samples each cycle. This ensures that the plotted waveform does not expand
or contract in the horizontal direction, improving user experience.
...
spectrum analysis on the samples with user-provided parameters. The triggering
algorithm uses hysteresis to find the first rising edge while ignoring rising
edges from noise. It does this by setting the hysteresis to 25\% of the signal
peak-to-peak so that only larger rising edges get triggered on \cite{hysteresis}.
The triggering function is provided below.

\setstretch{1}
\begin{minted}{rust}
    /// Returns position of first rising edge based on trigger
...
ideal for writing simple UIs quickly as it eliminates the need for complex
callbacks, UI objects, or state storage. \code{egui} was chosen to implement the
front end instead of JavaScript to avoid the overhead of message passing.
...
\begin{enumerate}
  ...
  \item Rising edge triggering with user-provided threshold
  \item Sample plotting with cursor and various pan and zoom options
  \item Plotting of samples in the frequency domain
  \item Exporting plotted samples to a CSV file
...
\subsection{Software Verification} \label{sec:soft-testing} % ERIC
...
Section \ref{sec:soft-testing}, developing a new design became the top priority.
